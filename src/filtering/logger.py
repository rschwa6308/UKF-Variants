...
class KF_Logger(Logger):
    ...
    def __init__(self, filter: KalmanFilter, T):
        super().__init(filter)
        ...
        self.cov_dim = self.cov_shape[0]*self.cov_shape[1]


class EKF_Logger(Logger):

    def __init__(self, filter: ExtendedKalmanFilter, T):
        super().__init(filter)
        system = filter.system
...
class UKF_Logger(Logger):
    ...
    def __init__(self, filter: UnscentedKalmanFilter, T):
        #super().__init(filter)
        system = filter.system
